            ...
            <div style={{ background: 'rgba(30,27,50,0.6)', borderRadius: '0 0 12px 12px', padding: '2rem' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                    <button
                        ...
                        disabled={!selectedRoot || feedback}
                        title={!selectedRoot ? "Please select a root word to continue" : ""}
                        style={{ background: (!selectedRoot || feedback) ? '#475569' : 'linear-gradient(135deg, #0d9488, #0891b2)', color: 'white', padding: '1rem 3rem', borderRadius: '12px', fontWeight: 'bold', fontSize: '1.1rem', cursor: (!selectedRoot || feedback) ? 'not-allowed' : 'pointer' }}
                    >
                        ➤ SUBMIT
                    </button>
                </div>
